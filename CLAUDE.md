...
- **Owner spec 13/09/2026: many bills, and bills withdrawn.**
  ...
  3. **Cancelling is the mirror entry, never an unposting - and the
     ...
     **Two traps.** It is a STORED COMPUTE over
     ...
     changing the fiscal country RESETS IT - which is exactly what
     happened on staging, and the next cancellation came out swapped
     with nobody told (owner, 14/09/2026). So
     `_clearance_ensure_storno()` now asserts the switch at the moment a
     reversal is raised and says so in the chatter on the one occasion
     it has to act. There is NO per-invoice alternative: `debit` and
     `credit` are computed from `balance` alone (`is_storno` is not even
     in their depends) and the credit note's receivable line is
     generated by Odoo's dynamic-line sync, not passed by us, so a flag
     set on the lines we build would miss it. And it is company-wide: every credit note the
