...
class AccountMove(models.Model):
    ...
    def _get_name_invoice_report(self):
        ...
        """
        self.ensure_one()
        if self.logistics_file_id and self.move_type in ('out_invoice',
                                                         'out_refund'):
    ...
    def _clearance_advances(self):
        ...
        if self._clearance_is_credit_note():
            # An advance was deducted on the invoice. Deducting it again
            # on the document that reverses that invoice would say the
            # client is owed money they were never charged.
    ...
    def _clearance_is_credit_note(self):
        """The same document, saying it is a credit note (owner
        19/09/2026): same layout, same blocks, same figures - the
        difference is the title, the reference to the invoice it
        reverses, and the reason it was raised."""
        self.ensure_one()
        return self.move_type == 'out_refund'

    def _clearance_document_title(self):
        self.ensure_one()
        if self._clearance_is_credit_note():
    ...
    def _clearance_raise_credit_note(self, lines, reason):
        ...
        The signs go the OTHER WAY ROUND, not negative (owner 19/09/2026,
        reversing the instruction of five days earlier): where the invoice
        debited the receivable and credited sales, the credit note debits
        sales and credits the receivable. That is what an out_refund does
        by itself, so all this has to do is NOT ask for storno accounting.
